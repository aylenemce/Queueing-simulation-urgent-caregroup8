...
mkdir(PictureFolder); %[output:8b373fa5]
%%
%[text] ## Set up
...
fprintf('P(%d) = %.6f\n', n, P(n+1)); %[output:63932898]
L_theory  = rho / (1 - rho);
Lq_theory = rho^2 / (1 - rho);
W_theory  = L_theory / lambda;
Wq_theory = Lq_theory / lambda;
theory = [L_theory, Lq_theory, W_theory, Wq_theory] %[output:2a74d1c0]
%%
...
for SampleNum = 1:NumSamples %[output:group:2fdc0e86]
    if mod(SampleNum, 10) == 0
        fprintf("%d ", SampleNum); %[output:17b50cc7]
    end
...
%fprintf("Mean number in system: %f\n", meanNumInSystem);
%%
H = [P(1)+P(2) %[output:group:8c16f53d] %[output:0c547ca8]
    P(3:end) ] %compute p_1 and p_0 %[output:group:8c16f53d] %[output:0c547ca8]
%%
...
hold(ax, "on");
plot(ax, 0:nMax-1, H, 'o', MarkerEdgeColor='k', MarkerFaceColor='r');
...
%TimeInSystem = vertcat(TimeInSystemSamples{:}); %[output:202708cd]
%%
%[text] ## Pictures and stats for time customers spend in the system
%[text] `Print out mean time spent in the system.`
meanTimeInSystem = mean(TimeInSystem); %[output:1666d0da]
...
%[text] Add titles and labels and such.
% Generate theoretical Wq samples
numTheorySamples = length(TimeWaiting);

% With probability (1 - rho), waiting time = 0
isZero = rand(numTheorySamples,1) < (1 - rho);

% Otherwise exponential with rate (mu - lambda)
Wq_theory_samples = zeros(numTheorySamples,1);
Wq_theory_samples(~isZero) = exprnd(1/(mu - lambda), sum(~isZero), 1);

% Plot histogram
histogram(ax, Wq_theory_samples, ...
    Normalization="probability", ...
    BinWidth=5/60, ...
    FaceAlpha=0.1);

legend(ax, "Simulation", "Theory (histogram)");
...
h = histogram(ax, TimeWaiting, Normalization = "probability", BinWidth = 1/12);
rho = lambda / mu;
% Generate theoretical Wq samples
numTheorySamples = length(TimeWaiting);
...
t = tiledlayout(fig,1,1);
ax = nexttile(t);
histogram(ax, CustomersServed, Normalization = "probability");
title(ax, "Customers Served per shift");
xlabel(ax, "Count");
ylabel(ax, "Frequency");
exportgraphics(fig, PictureFolder + filesep + "CustomersServed_histogram.pdf");
%%
...
%[text] Comparing values for P: exactly the same
%[text]     Theory: $P\[0-5\]=\[0.333, 0.222, 0.148, 0.0987, 0.0658, 0.0438\]$
%[text]     Simulation: $P\[0-5\]=\[0.333, 0.222, 0.148, 0.0987, 0.0658, 0.0438\]$
%[text] Comparing values for the mean number is system: 0.118437 difference between thoery and simulation
%[text]     Theory: $L=2$
%[text]     Simulation: $L=1.8881563$
%[text] Comparing values for the mean number waiting in system: 0.107954 difference between thoery and simulation
%[text]     Thoery: $L\_q= 1.333$
%[text]     Simulation: $L\_q=1.225046$
%[text] Comparing values for the mean time in system: 0.06362 difference between thoery and simulation
%[text]     Theory: $W=1$
%[text]     Simulation: $W=0.93638$
%[text] Comparing values for the mean waiting time is system: 0.066489
%[text]     Theory: $W\_q=0.666$
%[text]     Simulation: $W\_q=0.599511$

%[appendix]{"version":"1.0"}
%---
%[metadata:view]
%   data: {"layout":"inline","rightPanelPercent":17.2}
%---
%[output:8b373fa5]
...
%---
%[output:0c547ca8]
%   data: {"dataType":"matrix","outputData":{"columns":1,"name":"H","rows":10,"type":"double","value":[["0.5556"],["0.1481"],["0.0988"],["0.0658"],["0.0439"],["0.0293"],["0.0195"],["0.0130"],["0.0087"],["0.0058"]]}}
%---
%[output:718d4be3]
...
%---
%[output:202708cd]
%   data: {"dataType":"error","outputData":{"errorType":"runtime","text":"Brace indexing is not supported for variables of this type."}}
%---
%[output:1666d0da]
