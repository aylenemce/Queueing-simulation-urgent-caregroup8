...
% Use already-flattened data
TimeInSystem = TimeInSystemSamples;

meanTimeInSystemSamples = mean(TimeInSystem);
fprintf("Mean time in system: %f\n", meanTimeInSystemSamples);
...
% --- Theoretical points (like your Lq plot style) ---
tvals = linspace(0, max(TimeInSystem), 25);   % discrete points
fW = (mu - lambda) * exp(-(mu - lambda) * tvals);

% Scale by bin width to match histogram
plot(ax, tvals, fW * (5/60), 'o', ...
    MarkerEdgeColor='k', ...
    MarkerFaceColor='r');
...
%%
%[text] ## Theoretical calculations
%[text] Below shows all of the mathematical calculations for the theoretical calculations. We use different formulas to calculate L, L\_q, W, W\_q. 
%%
